...
def main(req: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        room_data = {
            'id': req_body.get('id'),
            'room_name': req_body.get('room_name'),
            'house_id': req_body.get('house_id'),
            'location': req_body.get('location'),
        }

        # Create the room entity in the Cosmos DB container
        container.create_item(body=room_data)

        return HttpResponse(
            json.dumps({'message': 'Rooms entity created successfully'}),
            status_code=201,
            headers={
                "Access-Control-Allow-Origin": "*",  # Add CORS header
                "Access-Control-Allow-Methods": "GET, POST, OPTIONS",  # Allow methods
